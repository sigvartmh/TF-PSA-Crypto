...
/* Worst-case size of a PAKE shared-key / implicit-key output across the
 * supported PAKEs (the J-PAKE intermediate buffer dominates the SPAKE2+
 * K_shared, which is at most a hash length). */
#define MBEDTLS_PSA_PAKE_BUFFER_SIZE MBEDTLS_PSA_JPAKE_BUFFER_SIZE
