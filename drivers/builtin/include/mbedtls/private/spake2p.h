 ...
 *
 * Message flow (RFC 9383 Appendix A.5):
 *
 *   Prover (w0,w1)                              Verifier (w0,L)
 *     write_key_share -> shareP   ------------>  read_key_share(shareP)
 *     read_key_share(shareV)      <------------  write_key_share -> shareV
 *                                 <------------  write_confirm -> confirmV
 *     read_confirm(confirmV)
 *     write_confirm -> confirmP   ------------>  read_confirm(confirmP)
 *     get_shared_key = K_shared                  get_shared_key = K_shared
 *
 * The key-share serialization is the SEC1 uncompressed point format; the
 * confirmation messages are the raw MAC tags.
 *
 ...
 *
 *                  The point is validated for group membership. Once both this
 *                  party's and the peer's shares are known, the shared values
 *                  Z and V, the transcript TT, and the key schedule are derived.
 *
 *                  Deriving Z and V requires elliptic-curve scalar
 *                  multiplications, so an RNG is needed for the side-channel
 *                  countermeasures even though no new secret is generated here.
 *
 * \param ctx       The SPAKE2+ context. This must be set up.
